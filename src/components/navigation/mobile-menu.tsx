    ...
    <>
      ...
      >
        <div className="flex flex-col px-5 pt-6 pb-12">
          ...
          {/* Primary Nav Links */}
          <nav aria-label="Mobile primary navigation" className="space-y-0.5 mb-6">
            {primaryNav.map((item) => (
              <Link
                key={item.href}
                href={item.href}
                onClick={onClose}
                className="block py-2 text-[11px] font-bold tracking-[0.14em] uppercase text-zinc-200 hover:text-cyan-400 min-h-[36px] flex items-center transition-colors"
              >
                {item.label}
              </Link>
            ))}
              ...
              return (
                ...
                >
                  <button
                    ...
                    className="t-acc-head w-full flex items-center justify-between px-3 py-2.5 text-left min-h-[40px]"
                        ...
                        {group.items.map((item) => (
                          <li key={item.href}>
                            <Link
                              href={item.href}
                              onClick={onClose}
                              className="block py-1.5 text-[10px] font-semibold text-zinc-300 hover:text-cyan-400 transition-colors"
                            >
                              <div className="uppercase">{item.label}</div>
                              {item.description && (
                                <div className="text-[8px] text-zinc-500 mt-0.5 uppercase tracking-wider">
                                  {item.description}
                                </div>
                              )}
                            </Link>
