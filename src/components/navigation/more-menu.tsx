  ...
  return (
    <div className={cn("relative", className)} ref={menuRef}>
      ...
      <div
        ...
        className={cn(
          "t-dropdown absolute top-full right-0 mt-2 w-[min(92vw,640px)] bg-[rgba(10,15,30,0.96)] border border-white/15 rounded-xl shadow-[0_20px_60px_rgba(0,0,0,0.5)] p-4 z-[80] grid grid-cols-3 gap-3 backdrop-blur-[24px]",
          isOpen && "is-open",
      ...
      >
        {[0, 1, 2].map((colIndex) => (
          <div key={colIndex} className="flex flex-col gap-3">
            {groups.filter((_, i) => i % 3 === colIndex).map((group) => (
              <div key={group.id} className="min-w-0">
                <p className="px-1.5 mb-1 text-[7px] font-bold uppercase tracking-[0.2em] text-cyan-400">
                  {group.label}
                </p>
                  ...
                  {group.items.map((item) => (
                    <li key={item.href}>
                      <Link
                        ...
                        className={cn(
                          "block rounded-lg px-1.5 py-1 transition-all group border border-transparent",
                          item.highlight
                      ...
                      >
                        <span className={cn(
                          "block text-[9px] font-bold tracking-wide uppercase",
                        ...
                        {item.description && (
                          <span className={cn(
                            "block text-[8px] leading-snug uppercase tracking-wider",
                            item.highlight ? "text-cyan-200/60 group-hover:text-cyan-100/80" : "text-zinc-500 group-hover:text-zinc-400"
                          )}>
                            {item.description}
                          </span>
                        )}
