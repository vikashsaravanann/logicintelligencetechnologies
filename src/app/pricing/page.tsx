  ...
  return (
    <PageShell>
      ...
      <div className="container py-32 md:py-40 max-w-7xl relative z-10">
        ...
        {/* Pricing Layout */}
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-16 xl:gap-8 mb-32">
          {PRICING_CONFIG.map((product, pIdx) => (
            <div key={product.id} className="flex flex-col gap-10">
              <h3 className="text-3xl font-black text-center text-white/90 tracking-tight uppercase">
                {product.name}
              </h3>
              <div className={`grid grid-cols-1 gap-8 ${product.plans.length > 1 ? 'md:grid-cols-2 xl:grid-cols-1' : ''}`}>
                {product.plans.map((plan, idx) => {
                  ...
                  return (
                    <motion.div 
                      ...
                      transition={{ delay: (pIdx * 0.1) + (idx * 0.1) }}
                      key={plan.name} 
                      className={`relative bg-black/40 backdrop-blur-xl border p-10 rounded-3xl flex flex-col h-full transition-all duration-300 hover:-translate-y-2 ${
                        isPopular 
                    ...
                    >
                      {isPopular && (
                        <div className="absolute -top-4 left-1/2 -translate-x-1/2 bg-gradient-to-r from-yellow-600 to-yellow-400 text-black text-[10px] font-black tracking-widest uppercase px-4 py-1.5 rounded-full shadow-lg">
                          Most Popular
                        </div>
                      )}
                      ...
                      <div className="flex-grow">
                        <p className="text-sm font-bold tracking-wider text-zinc-300 uppercase mb-6">
                          What's included
                        </p>
                      ...
                      <div className="mt-10 pt-8 border-t border-white/10">
                        <Link 
                          ...
                          className={`w-full flex items-center justify-center h-14 rounded-xl font-bold tracking-wide uppercase transition-all ${
                            isPopular
