...
function AuthContent() {
  ...
  const inputBase =
    "w-full pl-11 pr-4 py-3.5 bg-white/[0.04] border border-white/10 rounded-xl text-[14px] text-white placeholder:text-zinc-500 focus:outline-none focus:ring-2 focus:ring-primary/40 focus:border-primary/60 transition-all";

  const busy = isLoading || Boolean(oauthBusy);

  return (
    <main className="min-h-[100dvh] bg-[#0A0F1E] text-white flex">
      <section className="w-full lg:w-[46%] xl:w-[42%] flex flex-col justify-center px-5 sm:px-10 lg:px-14 py-10 relative">
        <div
        ...
        <div className="relative w-full max-w-[440px] mx-auto">
          ...
          <h1 className="text-3xl sm:text-[2.1rem] font-black tracking-tight mb-2">
            {mode === "signup" ? "Create your account" : mode === "forgot" ? "Reset password" : "Welcome back"}
          </h1>
          <p className="text-zinc-400 text-sm sm:text-[15px] mb-8 leading-relaxed">
            {mode === "signup"
              ? "Access projects, files, and support after you verify your email."
              : mode === "forgot"
          ...
          {serverError && (
            <div className="mb-5 p-3.5 rounded-xl bg-red-500/10 border border-red-500/20 text-red-200 text-sm flex gap-3">
              <Shield className="w-4 h-4 shrink-0 mt-0.5" />
              <p>{serverError}</p>
            </div>
          )}
          {serverSuccess && (
            <div className="mb-5 p-3.5 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-200 text-sm flex gap-3">
          ...
          <form onSubmit={mode === "forgot" ? handleForgot : handleSubmit} noValidate className="space-y-4">
                ...
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  {mode === "signin" && (
                    <button
                      ...
                      className="text-xs font-semibold text-primary hover:text-cyan-300"
                    >
                      Forgot?
                    </button>
                  )}
            ...
            <button
              type="submit"
              disabled={busy}
              className="w-full flex items-center justify-center gap-2 py-3.5 rounded-xl font-bold text-sm text-black bg-primary hover:brightness-110 transition-all disabled:opacity-50 mt-2"
            >
              {isLoading ? (
          ...
          <p className="mt-8 text-center text-sm text-zinc-500">
            {mode === "signin" && (
              <>
                ...
                <button
                  type="button"
                  className="text-primary font-semibold hover:text-cyan-300"
                  onClick={() => {
                    setMode("signup");
                    setServerError(null);
                    setServerSuccess(null);
      ...
      <aside className="hidden lg:flex flex-1 relative overflow-hidden border-l border-white/5 items-center justify-center p-12 xl:p-16">
        <div
        ...
        <div className="relative max-w-lg">
          <p className="text-[11px] font-bold uppercase tracking-[0.22em] text-primary mb-5">
            {COMPANY.entityLabel} · Coimbatore
          </p>
          <h2 className="text-4xl xl:text-[2.75rem] font-black leading-[1.15] tracking-tight mb-5">
            Production software.
            <span className="block text-zinc-400 font-semibold mt-1">Transparent delivery.</span>
          </h2>
          <p className="text-zinc-400 leading-relaxed mb-10">
            Track milestones, files, and support in one place. Built by{" "}
            {COMPANY.displayName} — web products and practical AI, with a free demo
            before you pay.
          </p>
          <ul className="space-y-3 text-sm text-zinc-300">
            {[
              "Project status and files in one portal",
              "Direct line to the team that writes the code",
              "Google, GitHub, or email sign-in",
            ].map((line) => (
...
export default function LoginPage() {
  return (
    <Suspense
      fallback={
        <div className="min-h-[100dvh] bg-[#0A0F1E] flex items-center justify-center text-zinc-400 text-sm">
          Loading portal…
        </div>
      }
    >
