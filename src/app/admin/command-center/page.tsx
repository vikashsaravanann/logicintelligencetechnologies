  ...
  return (
    <div className="min-h-screen bg-neutral-950">
      <div className="container mx-auto p-4 py-8 max-w-7xl space-y-8">
        ...
        {/* ── ALERTS ──────────────────────────────────────────── */}
        {voiceShieldPending > 0 && (
          <Link href="/admin/voiceshield-requests" className="flex items-center justify-between gap-4 px-5 py-4 rounded-2xl bg-cyan-950/40 border border-cyan-500/30 hover:bg-cyan-950/60 transition-all group">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-bold text-cyan-200">
                  {voiceShieldPending} VoiceShield Access Request{voiceShieldPending > 1 ? "s" : ""} Awaiting Approval
                </p>
                <p className="text-xs text-cyan-400/60">Review and grant console access to applicants</p>
              </div>
            </div>
        ...
        {(ticketsCount || 0) > 0 && (
          <Link href="/admin/support" className="flex items-center justify-between gap-4 px-5 py-4 rounded-2xl bg-rose-950/30 border border-rose-500/20 hover:bg-rose-950/50 transition-all group">
            <div className="flex items-center gap-3">
              <div className="w-9 h-9 rounded-xl bg-rose-500/10 flex items-center justify-center shrink-0">
                <AlertCircle className="w-4 h-4 text-rose-400" />
              </div>
              <div>
                <p className="text-sm font-bold text-rose-200">{ticketsCount} Open Support Ticket{(ticketsCount || 0) > 1 ? "s" : ""} Need Attention</p>
                <p className="text-xs text-rose-400/60">Respond to customer issues to maintain SLA</p>
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          {/* Recent Leads Feed - spans 2 cols */}
          <div className="lg:col-span-2 rounded-2xl border border-neutral-800 bg-neutral-900/50">
            ...
            <div className="divide-y divide-neutral-800/60">
              {leads && leads.length > 0 ? (
                leads.map((l) => (
                  ...
                  >
                    <div className="flex items-center gap-3 min-w-0">
                      <div className="w-9 h-9 rounded-xl bg-neutral-800 flex items-center justify-center shrink-0 text-sm font-bold text-zinc-400 group-hover:bg-primary/10 group-hover:text-primary transition-colors">
                        {l.name?.[0]?.toUpperCase() || "?"}
                      </div>
                      <div className="min-w-0">
                        <div className="font-bold text-white text-sm truncate group-hover:text-primary transition-colors">{l.name}</div>
                        <div className="text-xs text-zinc-500 flex items-center gap-2 mt-0.5 flex-wrap">
                ...
                <p className="text-xs text-zinc-600 py-10 text-center">No leads yet. Share your website to start capturing contacts.</p>
              )}
            </div>
          </div>
          ...
          <div className="flex flex-col gap-6">
                  ...
                  recentBookings.map((b) => (
                    <div key={b.id} className="p-4">
                      ...
                      <div className="text-xs text-zinc-500 flex items-center gap-1.5 mt-1">
                        ...
                        {new Date(b.slot_time).toLocaleDateString("en-IN", { day: "numeric", month: "short" })} at {new Date(b.slot_time).toLocaleTimeString("en-IN", { hour: "2-digit", minute: "2-digit" })}
                      </div>
                      <div className="text-[10px] text-primary font-medium mt-1">{b.consultation_type}</div>
                    </div>
                  ))
                ) : (
                  <p className="text-xs text-zinc-600 py-6 text-center px-4">No upcoming bookings.</p>
                )}
              </div>
            </div>
            ...
            <div className="rounded-2xl border border-neutral-800 bg-neutral-900/50">
              <div className="flex items-center justify-between p-5 border-b border-neutral-800">
                ...
                <Link href="/admin/support" className="text-xs text-primary font-bold hover:underline">
                  All →
                </Link>
              </div>
              <div className="divide-y divide-neutral-800/60">
                {openTickets && openTickets.length > 0 ? (
                  openTickets.map((t) => (
                    <div key={t.id} className="p-4">
                      <div className="font-bold text-white text-sm truncate">{t.subject || "Untitled Ticket"}</div>
                      <div className="text-[10px] text-zinc-500 mt-1">{t.email}</div>
                      <div className="flex items-center gap-2 mt-1.5">
                        <span className="px-2 py-0.5 rounded-full text-[9px] font-bold uppercase bg-rose-500/10 text-rose-400 border border-rose-500/20">Open</span>
                  ...
                  <div className="p-6 text-center">
                    ...
                    <p className="text-xs text-zinc-600">All tickets resolved.</p>
                  </div>
                )}
              </div>
            </div>
